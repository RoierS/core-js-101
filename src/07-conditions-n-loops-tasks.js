...
function doRectanglesOverlap(rect1, rect2) {
  ...
  const isHorisontalGap = rectRight1 < rect2.left || rectRight2 < rect1.left;

  const isVerticalGap = rectBottom1 < rect2.top || rectBottom2 < rect1.top;

  return !(isHorisontalGap || isVerticalGap);
}
...
function isInsideCircle(circle, point) {
  const distanceCenterToPoint = Math.sqrt(
    ((point.x - circle.center.x) ** 2)
    + ((point.y - circle.center.y) ** 2),
  );

  return distanceCenterToPoint < circle.radius;
}
...
function findFirstSingleChar(str) {
  ...
  const res = chars.filter((char) => chars.indexOf(char) === chars.lastIndexOf(char));
...
function getCommonDirectoryPath(pathes) {
  const directoryParts = pathes[0].split('/');
  const commonParts = [];

  directoryParts.forEach((part, index) => {
    if (pathes.every((path) => path.split('/')[index] === part)) {
